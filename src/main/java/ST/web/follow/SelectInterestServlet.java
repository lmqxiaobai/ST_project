...
public class SelectInterestServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        // 接收前端的参数
        String follower_id = req.getParameter("follower_id");
        // 对user_id进行转换
        if(follower_id != null) {
            List<User> users = followService.getInterestFollow(Integer.parseInt(follower_id));
            // 通过每个用户的id去获取头像路径
            for(User user : users){
            ...
            req.getRequestDispatcher("/wb_InterestFollow.jsp").forward(req, resp);
        }
    }
